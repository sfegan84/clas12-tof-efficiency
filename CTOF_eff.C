...
// Particle numbers for 2pi events
 Int_t negative, positive, nonelectron, ft_electron;
  ...
  for(Int_t i=0;i<files->GetEntries();i++){
    ...
    while(c12.next()==true){
	  ...
	  negative++;
	    ...
	    // electrons not in FT are skipped
	    switch(p->getRegion()) {
...
// 	      //cout << "FD" << endl;
      ...
      //if(ft_electron>0){//Use continue statement to skip events with FT electron
      //if(ft_electron<1){//Use continue statement to skip events without FT electron
	//continue;
      //}
...
// //	   if(fabs(DeltaP) > 1.0) continue;
// //	   if(fabs(DeltaTheta) > 50) continue;
// //	   if(fabs(DeltaPhi) > 50) continue;
...
{	  
	  ...
	  for(auto& p : particles){
		...
		//std::cout << "first CD particle" << p->par()->getPid() << std::endl;
	      ...
	      if(trackMom<0.4) continue;  //cut at particle momentum less than 400 MeV - threshold of reliable CND hit
	      ...
	      //Be it measured or calculated, any meaningful beta cut leads to efficiencies of 100%
