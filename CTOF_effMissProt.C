...
Double_t DeltaP, DeltaTheta, DeltaPhi;
  ...
  //TFile fileOutput1("/u/home/sfegan/CTOF_Efficiency_RGA_FALL2018_testMissPr5000s.root","recreate");
  TFile fileOutput1("/home/stuart/CLAS/CTOF/CTOF_Efficiency_RGA_FALL2018_test5038MissPr.root","recreate");
...
auto* hmass=new TH1F("prmmass","Missing Mass e' #pi^{-} #pi^{+};MM(e'#pi^{-}#pi^{+}) [GeV];Counts",300,-1,2);
auto* hmass2=new TH1F("prmmass2","Missing Mass e' #pi^{-} #pi^{+} (post cuts);MM(e'#pi^{-}#pi^{+}) [GeV];Counts",300,-1,2);
auto* hdeltaP=new TH1F("DeltaMomentum","Momentum difference of proton detected and reconstructed;#Delta P [GeV];Counts",400,-2,2);
auto* hdeltaTheta=new TH1F("DeltaTheta","#theta difference of proton detected and reconstructed;#Delta #theta [deg];Counts",360,-180,180);
auto* hdeltaPhi=new TH1F("DeltaPhi","#phi difference of proton detected and reconstructed;#Delta #phi [deg];Counts",360,-180,180);
...
//   }
  ...
  for(Int_t i=0;i<files->GetEntries();i++){
	...
	//nonelectron reappropriated to refer to 'missing' particle	
      ...
      // Getting 2pi events
       if(nonelectron == 1 && electrons.size() == 1 && pims.size() == 1 && pips.size() == 1){
 	SetLorentzVector(el,electrons[0]);
         //SetLorentzVector(pr,protons[0]);
         SetLorentzVector(pip,pips[0]);
         SetLorentzVector(pim,pims[0]);
         misspr = beam + target - el - pim - pip;
         hmass->Fill(misspr.M2());
	
         // Cut on missing mass of the pi^-
         if((misspr.M2() > 0.702) && (misspr.M2() < 1.077)){
           DeltaP = misspr.Rho() - pr.Rho();
           DeltaTheta = TMath::RadToDeg()* (misspr.Theta() - pr.Theta());
           DeltaPhi = TMath::RadToDeg()* (misspr.Phi() - pr.Phi());
...
void CTOF_eff(){
  ...
  // TString inFile2("/volatile/clas12/rg-b/production/recon/spring2019/torus-1/pass1/v0/dst/train/inc/*.hipo");
